...
class Ledbar
{
  public:
    ...
    bool setImage2(const uint32_t* pColor, uint8_t colorSize, const uint16_t* pImage, uint8_t width, uint8_t height);
    bool setImage3(const uint32_t* pColor, uint8_t colorSize, uint16_t pos, uint8_t width, uint8_t height);
    bool setTime(int yy, uint8_t m, uint8_t dd, uint8_t hh, uint8_t mm, uint8_t ss);
